...
{
   return [self UnzipFileTo:path overWrite:overwrite isEpub:NO];
}


-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite isEpub:(BOOL)isepub{
    BOOL success = YES;
    int index = 0;
    int progress = -1;
    ...
    do{
        @autoreleasepool {
            ...
            //2015.04.10 重新修改中文解压引起的问题。
            NSString *strPath = nil;
            if(isepub)
            {
                strPath = [NSString  stringWithCString:filename encoding:self.stringEncoding];
                if (strPath.length == 0) {
...
-(NSInteger) addFolderToZip:(NSString*)path pathPrefix:(NSString*)prefix {
	...
	for (NSInteger i=0; i<[dirArray count]; i++) {
